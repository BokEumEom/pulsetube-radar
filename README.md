...
- TOP 10, 게시 이후 평균 조회 속도, 최근 공개 메뉴
...
Cloudflare Worker의 `YT_API_KEY` Secret이 설정되면 `/api/youtube/trending`이 대한민국 인기 영상 25개를 서버에서 조회하고 15분간 엣지 캐시합니다. `category` 파라미터를 사용하면 음악·게임·엔터테인먼트·뉴스/정치·스포츠·영화/애니메이션·과학기술·코미디를 별도로 조회합니다. API 키는 브라우저에 노출되지 않습니다. 순위 변화·신규 진입·7일 시계열은 D1 예약 수집을 연결한 다음 단계에서 활성화합니다.
