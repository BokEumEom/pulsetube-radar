...
- TOP 10, 게시 이후 평균 조회 속도, 최근 공개·카테고리 행
- 실데이터 연결 실패 시 샘플 스냅샷 자동 폴백
- 현재 스냅샷 카테고리 점유율
- 취향 퀴즈와 10종 컬러 테마
- 데스크톱·모바일 반응형 UI

Cloudflare Worker의 `YT_API_KEY` Secret이 설정되면 `/api/youtube/trending`이 대한민국 인기 영상 25개를 서버에서 조회하고 15분간 엣지 캐시합니다. API 키는 브라우저에 노출되지 않습니다. 순위 변화·신규 진입·7일 시계열은 D1 예약 수집을 연결한 다음 단계에서 활성화합니다.
